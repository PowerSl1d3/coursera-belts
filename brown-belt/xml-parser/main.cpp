...
vector<Spending> LoadFromXml(istream& input) {
    ...
    std::vector<Spending> vec_spending;

    vec_spending.reserve(root.Children().size());

    for (const auto& element : root.Children()) {
        vec_spending.push_back({
            element.AttributeValue<std::string>("category")
                , element.AttributeValue<int>("amount")
                });
    }
