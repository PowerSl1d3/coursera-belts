...
public:
  ...
  AirportCounter() {
    size_t end = static_cast<size_t>(TAirport::Last_);
    for (size_t i = 0; i < end; i++) {
      airports[static_cast<TAirport>(i)] = 0;
    }
  }
  ...
  Items GetItems() const {
    return {airports.begin(), airports.end()};
  }

private:
  std::map<TAirport, uint32_t> airports;
};*/
...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) {
    while (begin != end) {
      airports[static_cast<uint32_t>(*begin++)]++;
    }
  }
  ...
  // упорядоченных по аэропорту
  Items GetItems() const {
    Items airports_to_pair;
    for (size_t i = 0; i < airports.size(); i++) {
      airports_to_pair[i] = make_pair(static_cast<TAirport>(i), airports[i]);
    }
    return airports_to_pair;
  }

private:
  std::array<uint32_t, static_cast<uint32_t>(TAirport::Last_)> airports = {};
};
